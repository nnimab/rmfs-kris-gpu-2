...
class Warehouse:
    ...
    def tick(self):
        try:
            # V5.0: 計算揀貨台排隊長度
            self.picking_station_queue_length = 0
            for station in self.station_manager.getAllStations():
                if station.isPickerStation():
                    # 統計分配給此揀貨台的機器人數量
                    self.picking_station_queue_length += len(station.robot_ids)
            ...
            if len(self.job_queue) > 0:
                ...
                if nearest_robot is not None:
                    ...
                    # --- 【錯誤修復】將機器人ID關聯到訂單 ---
                    for order_id, _, _ in job.orders:
                        order_to_update = self.order_manager.getOrderById(order_id)
                        if order_to_update:
                            order_to_update.robot_id = nearest_robot.id
                    # --- 【修復結束】 ---
            ...
            self.total_energy += total_energy  # \u7d2f\u52a0\u800c\u975e\u8986\u84cb
            ...
            # 記錄錯誤但不中斷執行
            import traceback
            print(f'WARNING: Error in tick {self._tick}: {str(e)}')
            traceback.print_exc()
            # 繼續執行，增加 tick
            self._tick += TICK_TO_SECOND
